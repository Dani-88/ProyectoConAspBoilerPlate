...
using ABM_Test.Migrations.SeedData;
...
namespace ABM_Test.Tests
{
    public abstract class ABM_TestTestBase : AbpIntegratedTestBase<ABM_TestTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected ABM_TestTestBase()
        {
        ...
        protected void UsingDbContext(Action<ABM_TestDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ABM_TestDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ABM_TestDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ABM_TestDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ABM_TestDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABM_TestDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ABM_TestDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABM_TestDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ABM_TestDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ABM_TestDbContext, Task<T>> func)
        {
            T result;
